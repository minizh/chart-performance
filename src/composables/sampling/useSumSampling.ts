...
/**
 * 求和采样 - 将数据分成桶，取每个桶的 Y 值之和
 * @param data - 原始数据点数组
 * @param params - 采样参数
 * @param defaultMaxPoints - 默认最大点数
 * @returns 采样后的数据点数组
 */
export const useSumSampling: SamplingFunction = (
  data: DataPoint[],
  ...
  for (let i = 0; i < targetSize; i++) {
    ...
    const sumY = bucket.reduce((sum, p) => sum + (p[yDim] as number), 0)

    const sumPoint: DataPoint = { ...bucket[0] }
    sumPoint[xDim] = avgX
    sumPoint[yDim] = sumY
    result.push(sumPoint)
  }

  return result
}

export default useSumSampling
